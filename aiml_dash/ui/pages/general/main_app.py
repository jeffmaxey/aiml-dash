...
app.layout = dbc.Container(
    [
        nav.get_navbar(),
        dbc.Row(
            [html.Div([nav.get_sidebar(), html.Div(id="id-page-content", className="content")])],
        ),
        # stores
...
def route_page(pathname):
    ...
    if pathname == "/":
        return landing.get_layout()
    elif pathname == gs.nav_experiment_path:
        return experiment.get_layout()
    elif pathname == gs.nav_upload_path:
        return upload.get_layout()
...
def on_load_search(results_are_cleared, n_clicks, query_sequence, threshold, n_top_hot_cold):
        ...
        try:
            ...
            if settings.is_sequence_alignment_profiling_enabled():
                utils.log_with_context(
                    f"[PROFILING] on_load_search: get_all_lab_sequences() {time.time() - start_time} s",
            ...
            if settings.is_sequence_alignment_profiling_enabled():
                utils.log_with_context(
                    f"[PROFILING] on_load_search: get_alignments() {time.time() - start_time} s",
                    log_flag=settings.is_sequence_alignment_profiling_enabled(),
                )
                start_time = time.time()
            ...
            if settings.is_sequence_alignment_profiling_enabled():
                utils.log_with_context(
                    f"[PROFILING] on_load_search: finding gof/lof {time.time() - start_time} s",
                    log_flag=settings.is_sequence_alignment_profiling_enabled(),
                )
            ...
            return (
                ...
                info,
                styles.display_block,  # set the stylesibility on
...
def on_button_run_seq_matching(n_clicks, results_are_cleared):
    ...
    if ctx.triggered_id == "id-button-run-seq-matching" and n_clicks > 0 and not results_are_cleared:
        return (
            # clear everything
            [],  # clear alert if any
            styles.display_none,  # clear seq_alignment page results
...
def display_default_selected_search(data):
...
def display_selected_search(selected_rows):
        ...
        if geometry_file:
            ...
            viewer = [
                dash_molstar.MolstarViewer(
                    data=pdb_cif,
                    style={"width": "auto", "height": styles.seq_match_protein_viewer_height},
                    # focus=analyse,
                )
            ]
        else:
...
def redirect_to_experiment_page(n_clicks):
    ...
    if n_clicks != 0 and ctx.triggered_id == "id-button-goto-experiment":
        return gs.nav_experiment_path, experiment.get_layout()
    else:
        raise PreventUpdate
...
def on_load_experiment_page(pathname, experiment_id):
    ...
    Retrieves experiment data, metadata, generates stylesualizations (heatmap, rank/SSM plot),
...
def on_button_run_exp_related_variants(n_clicks, results_are_cleared):
    ...
    if ctx.triggered_id == "id-button-run-seq-matching-exp" and n_clicks > 0 and not results_are_cleared:
        return (  # clear everything
            [],  # clear alert if any
            styles.display_none,  # seq_alignment page results
            True,  # set clear to true
        )
    else:
        raise PreventUpdate
        ...
        if selected_experiment_geometry_file and experiment_geometry_file:
            ...
            selected_experiment_viewer = [
                dash_molstar.MolstarViewer(
                    ...
                    style={"width": "auto", "height": styles.related_protein_viewer_height},
                    # focus=analyse,
                )
            ]
