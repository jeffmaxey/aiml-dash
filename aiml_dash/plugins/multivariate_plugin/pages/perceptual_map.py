...
def create_perceptual_map(
    n_clicks, dataset_name, row_var, col_var, value_var, n_dims, normalize
):
    ...
    value_var : Any
        Input value for ``value_var``.
    n_dims : Any
        Input value for ``n_dims``.
    normalize : Any
        Value provided for this parameter."""
    if not all([dataset_name, row_var, col_var]):
        return (
            {},
            dmc.Text(
                "Please select dataset, row variable, and column variable.", c="red"
            ),
            "",
            {},
            None,
    ...
    try:
        ...
        summary = dmc.Stack(
            [
                ...
                *[
                    dmc.Text(f"Dimension {i + 1}: {var:.2f}%")
                    for i, var in enumerate(explained_variance[:n_dims])
                ],
                dmc.Text(
                ...
                dmc.TabsPanel(
                    [
                        dag.AgGrid(
                            ...
                            columnDefs=[
                                {"field": col, "flex": 1}
                                for col in row_coord_df.reset_index().columns
                            ],
                            defaultColDef={
                                "sortable": True,
                                "filter": True,
                ...
                dmc.TabsPanel(
                    [
                        dag.AgGrid(
                            ...
                            columnDefs=[
                                {"field": col, "flex": 1}
                                for col in col_coord_df.reset_index().columns
                            ],
                            defaultColDef={
                                "sortable": True,
                                "filter": True,
