...
def run_factor_analysis(n_clicks, dataset_name, variables, n_factors, rotation, cutoff):
    ...
    try:
        ...
        # Factor analysis
        fa = FactorAnalysis(
            n_components=n_factors, rotation=rotation if rotation != "none" else None
        )
        fa.fit(X_scaled)
        ...
        for col in loadings_display.columns:
            loadings_display[col] = loadings_display[col].apply(
                lambda x: f"{x:.3f}" if abs(x) >= cutoff else ""
            )
