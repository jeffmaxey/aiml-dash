                                ...
                                [
                                    dmc.Stack(
                                        [
                                            ...
                                            dmc.Text(
                                                "Estimation Method", size="sm", fw=500
                                            ),
...
def run_conjoint_analysis(
    ...
    id_var : Any
        Input value for ``id_var``.
    response_var : Any
        Input value for ``response_var``.
    attributes : Any
        Input value for ``attributes``.
    method : Any
        Input value for ``method``.
    include_interactions : Any
        Input value for ``include_interactions``.
    by_respondent : Any
        Value provided for this parameter."""
    if not all([dataset_name, response_var, attributes]):
        return (
            ...
            {},
            dmc.Text(
                "Please select dataset, response variable, and attributes.", c="red"
            ),
            "",
            None,
            dmc.Notification(
    ...
    try:
        ...
        if include_interactions and len(attributes) >= 2:
            for attr1, attr2 in combinations(attributes, 2):
                ...
                X_dummies[interaction_col] = (
                    df[attr1].astype(str) + "_" + df[attr2].astype(str)
                )
        ...
        for attr in attributes:
            ...
            attr_cols = [
                col for col in coefficients.index if col.startswith(f"{attr}_")
            ]

            if attr_cols:
                # Get part-worths (include base level = 0)
                attr_partworths = {
                    col.replace(f"{attr}_", ""): coefficients[col] for col in attr_cols
                }

                # Add base level
                levels = df[attr].unique()
                base_level = [
                    lvl for lvl in levels if f"{attr}_{lvl}" not in X_dummies.columns
                ]
                if base_level:
        ...
        if total_importance > 0:
            importance_pct = {
                k: (v / total_importance) * 100 for k, v in importance.items()
            }
        else:
        ...
        summary = dmc.Stack(
            [
                ...
                *[
                    ...
                    for attr, imp in sorted(
                        importance_pct.items(), key=lambda x: x[1], reverse=True
                    )
                ],
            ],
            gap="xs",
