...
def update_strata_options(method):
    ...
    if method == "stratified":
        ...
        if dataset_names:
            ...
            df = data_manager.get_dataset(first_dataset)
            if df is not None:
    ...
    try:
        ...
        if method == "random":
            ...
            sample_df = df.groupby(strata_var, group_keys=False).apply(
                lambda x: x.sample(
                    n=max(1, int(size * len(x) / len(df))), random_state=seed
                )
            )
        ...
        data_manager.add_dataset(
            f"{dataset_name}_sample", sample_df, f"Sample from {dataset_name}"
        )
                        ...
                        dmc.Card(
                            [
                                dmc.Stack(
                                    [
                                        dmc.Text(
                                            "Original dataset", size="sm", c="dimmed"
                                        ),
                            ...
                            [
                                dmc.Stack(
                                    [
                                        ...
                                        dmc.Text(
                                            f"{len(sample_df)} observations", fw=500
                                        ),
                                    ],
                                    gap=0,
                                    align="center",
                        ...
                        dmc.Card(
                            [
                                dmc.Stack(
                                    [
                                        dmc.Text(
                                            "Sampling rate", size="sm", c="dimmed"
                                        ),
                                        dmc.Text(
                ...
                dmc.Card(
                    [
                        dmc.Stack(
                            [
                                dmc.Text(
                                    "Sample Preview (first 10 rows)", size="sm", fw=500
                                ),
                                dmc.Table(
                                    striped=True,
