...
def manage_factors(
    add_clicks, remove_clicks, factor_name, level1, level2, max_levels, factors
):
    ...
    remove_clicks : Any
        Input value for ``remove_clicks``.
    factor_name : Any
        Input value for ``factor_name``.
    level1 : Any
        Input value for ``level1``.
    level2 : Any
        Input value for ``level2``.
    max_levels : Any
        Input value for ``max_levels``.
    factors : Any
...
def create_design(n_clicks, factors, interactions, n_trials, seed):
    ...
    factors : Any
        Input value for ``factors``.
    interactions : Any
        Input value for ``interactions``.
    n_trials : Any
        Input value for ``n_trials``.
    seed : Any
        Value provided for this parameter."""
    if not factors:
    ...
    try:
        ...
        if interactions:
            for interaction in interactions:
                ...
                if (
                    len(parts) == 2
                    and parts[0] in df.columns
                    and parts[1] in df.columns
                ):
                    df[interaction] = (
        ...
        table = dag.AgGrid(
            ...
            columnDefs=[
                {"field": col, "sortable": True, "filter": True} for col in df.columns
            ],
            defaultColDef={
                "resizable": True,
                "sortable": True,
