...
def generate_randomization(
    n_clicks, n_subjects, n_groups, group_names_text, method, seed
):
    ...
    n_subjects : Any
        Input value for ``n_subjects``.
    n_groups : Any
        Input value for ``n_groups``.
    group_names_text : Any
        Input value for ``group_names_text``.
    method : Any
    ...
    try:
        ...
        if group_names_text and group_names_text.strip():
            group_names = [
                name.strip() for name in group_names_text.split("\n") if name.strip()
            ]
        ...
        # Create DataFrame
        df = pd.DataFrame(
            {"subject_id": range(1, n_subjects + 1), "group": assignments}
        )
                        ...
                        dmc.Card(
                            [
                                dmc.Stack(
                                    [
                                        dmc.Text(
                                            group, size="sm", style={"color": "gray"}
                                        ),
                                        dmc.Text(
                                            f"{count} subjects",
