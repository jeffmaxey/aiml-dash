...
def toggle_connection_modal(new_clicks, save_clicks, cancel_clicks, is_open):
    ...
    ----------
    new_clicks : Any
        Input value for ``new_clicks``.
    save_clicks : Any
        Input value for ``save_clicks``.
    cancel_clicks : Any
        Input value for ``cancel_clicks``.
    is_open : Any
        Value provided for this parameter."""
    return not is_open
...
def save_connection(
    n_clicks, name, dialect, server, database, username, password, port
):
...
def execute_query(exec_clicks, preview_clicks, connection_name, query, dataset_name):
    ...
    ----------
    exec_clicks : Any
        Input value for ``exec_clicks``.
    preview_clicks : Any
        Input value for ``preview_clicks``.
    connection_name : Any
        Input value for ``connection_name``.
    query : Any
    ...
    try:
            ...
            return dmc.Stack(
                [
                    ...
                    dag.AgGrid(
                        ...
                        columnDefs=[
                            {"field": col, "filter": True} for col in df.columns
                        ],
        ...
        return dmc.Stack(
            [
                dmc.Text(
                    f"{len(df)} rows × {len(df.columns)} columns", size="sm", c="dimmed"
                ),
                dag.AgGrid(
                    rowData=df.head(100).to_dict("records"),
