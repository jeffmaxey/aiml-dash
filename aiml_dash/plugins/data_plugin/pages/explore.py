...
def calculate_statistics(n_clicks, dataset_name, vars, byvar, functions, data_filter):
    ...
    vars : Any
        Input value for ``vars``.
    byvar : Any
        Input value for ``byvar``.
    functions : Any
    ...
    if not n_clicks or not dataset_name or not vars or not functions:
        return dmc.Center(
            dmc.Text(
                "Select variables and functions, then click Calculate", c="dimmed"
            ),
            style={"height": "300px"},
        )
...
def export_statistics(n_clicks, dataset_name, vars, byvar, functions, data_filter):
    ...
    try:
        ...
        return dcc.send_data_frame(
            result.to_csv, f"{dataset_name}_explore.csv", index=False
        )
    except:
        return dash.no_update
