...
def create_pivot_table(
    n_clicks, dataset_name, rows, cols, values, aggfunc, margins, normalize, data_filter
):
    ...
    cols : Any
        Input value for ``cols``.
    values : Any
        Input value for ``values``.
    aggfunc : Any
        Input value for ``aggfunc``.
    margins : Any
        Input value for ``margins``.
    normalize : Any
        Input value for ``normalize``.
    data_filter : Any
        Value provided for this parameter."""
                        ...
                        dmc.SimpleGrid(
                            [
                                ...
                                dmc.Card(
                                    [
                                        dmc.Text(
                                            "Degrees of Freedom", size="sm", c="dimmed"
                                        ),
                                        dmc.Text(
                                            f"{chi2_results['dof']}", size="xl", fw=700
                                        ),
                                    ],
                                    withBorder=True,
                                    p="sm",
...
def export_pivot(
    ...
    try:
        ...
        return dcc.send_data_frame(
            pivot.reset_index().to_csv, f"{dataset_name}_pivot.csv", index=False
        )
    except:
        return dash.no_update
