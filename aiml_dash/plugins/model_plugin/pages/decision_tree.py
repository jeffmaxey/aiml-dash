...
def layout():
    """Create the decision tree page layout."""
            ...
            dmc.Tabs(
                [
                    dmc.TabsList(
                        [
                            ...
                            dmc.TabsTab(
                                ...
                                value="plot",
                                leftSection=DashIconify(
                                    icon="carbon:tree-view", width=16
                                ),
                            ),
                        ]
                    ),
                    ...
                    dmc.TabsPanel(
                        [
                            dmc.Card(
                                [html.Div(id="cart-summary")], withBorder=True, p="md"
                            ),
                        ],
                        value="summary",
                    ),
                    dmc.TabsPanel(
                        [
                            dmc.Card(
                                [
                                    dcc.Graph(
                                        id="cart-plot", style={"height": "700px"}
                                    ),
                                ],
                                withBorder=True,
                                p="md",
...
def update_datasets(_):
    """Populate the dataset selector with available dataset names.
...
def update_variables(dataset):
    """Update response and explanatory variable choices for the selected dataset.
...
def build_tree(n, dataset, response, explanatory, tree_type, max_depth, min_samples):
    """Train a CART model and return model metadata, results, and status UI.
    ...
    tree_type : Any
        Input value for ``tree_type``.
    max_depth : Any
        Input value for ``max_depth``.
    min_samples : Any
        Value provided for this parameter."""
...
def update_summary(results):
    """Render summary information for the most recently trained tree.
...
def update_plot(results):
    """Render placeholder decision tree visualization output.
