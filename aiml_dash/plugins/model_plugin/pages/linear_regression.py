...
def estimate_model(
    n_clicks, dataset_name, response, explanatory, options, interactions
):
...
def update_summary(model_data, show_rmse, show_vif, show_ci):
    ...
    ----------
    model_data : Any
        Input value for ``model_data``.
    show_rmse : Any
        Input value for ``show_rmse``.
    show_vif : Any
        Input value for ``show_vif``.
    show_ci : Any
        Value provided for this parameter."""
    if not model_data:
        return dmc.Alert(
            "No model estimated yet. Go to Model tab to estimate.", color="yellow"
        )
    ...
    summary_parts = [
        dmc.Group(
            [
                ...
                dmc.Badge(
                    f"Adj. R² = {model_data['adj_r2']:.4f}", color="cyan", size="lg"
                ),
...
def generate_predictions(n_clicks, model_data, pred_type, pred_dataset, pred_cmd):
    ...
    try:
        if pred_type == "data":
            ...
            return dmc.Alert(
                "Prediction from dataset completed (placeholder)", color="green"
            )
