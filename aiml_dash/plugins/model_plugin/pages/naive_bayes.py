                    ...
                    dmc.TabsList(
                        [
                            ...
                            dmc.TabsTab(
                                ...
                                value="predict",
                                leftSection=DashIconify(
                                    icon="carbon:chart-bubble", width=16
                                ),
                            ),
                            dmc.TabsTab(
                                "Plot",
                                ...
                                [
                                    ...
                                    dmc.GridCol(
                                        [
                                            dmc.Card(
                                                [
                                                    dmc.Title("Model Status", order=4),
                        ...
                        [
                            dmc.Card(
                                [
                                    dmc.Stack(
                                        [
                                            dmc.Title("Model Diagnostics", order=4),
...
def update_dataset_list(_):
    ...
    datasets = [
        {"label": name, "value": name} for name in data_manager.get_dataset_names()
    ]
...
def estimate_naive_bayes(
    n_clicks, dataset_name, response, explanatory, nb_type, smoothing, train_split, seed
):
    ...
    response : Any
        Input value for ``response``.
    explanatory : Any
        Input value for ``explanatory``.
    nb_type : Any
        Input value for ``nb_type``.
    smoothing : Any
        Input value for ``smoothing``.
    train_split : Any
        Input value for ``train_split``.
    seed : Any
        Value provided for this parameter."""
...
def update_summary(results, model_config):
    ...
    ----------
    results : Any
        Input value for ``results``.
    model_config : Any
        Value provided for this parameter."""
    if not results or not model_config:
        return dmc.Text(
            "No model estimated yet. Go to Model tab to estimate.", c="dimmed"
        )
                    ...
                    dmc.Card(
                        [
                            dmc.Stack(
                                [
                                    dmc.Text(
                                        "Training Accuracy", size="sm", c="dimmed"
                                    ),
                                    dmc.Title(
                                        f"{results['train_accuracy']:.3f}", order=3
                                    ),
                                ],
                                gap=0,
                                align="center",
                        ...
                        [
                            dmc.Stack(
                                [
                                    ...
                                    dmc.Title(
                                        f"{results['test_accuracy']:.3f}", order=3
                                    ),
                                ],
                                gap=0,
                                align="center",
...
def generate_predictions(n_clicks, pred_dataset, pred_name, model_config):
    ...
    n_clicks : Any
        Input value for ``n_clicks``.
    pred_dataset : Any
        Input value for ``pred_dataset``.
    pred_name : Any
        Input value for ``pred_name``.
    model_config : Any
        Value provided for this parameter."""
    if not model_config:
