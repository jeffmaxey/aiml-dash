    ...
    if dist_type == "normal":
        return dmc.Stack(
            [
                ...
                dmc.NumberInput(
                    id="sim-param-2", label="Std Dev", value=10, step=1, min=0
                ),
            ],
            gap="xs",
        )
...
def manage_variables(add_clicks, clear_clicks, var_name, dist, p1, p2, p3, vars_data):
    ...
    ----------
    add_clicks : Any
        Input value for ``add_clicks``.
    clear_clicks : Any
        Input value for ``clear_clicks``.
    var_name : Any
        Input value for ``var_name``.
    dist : Any
        Input value for ``dist``.
    p1 : Any
        Input value for ``p1``.
    p2 : Any
        Input value for ``p2``.
    ...
    if triggered_id == "sim-add-var-btn" and var_name:
        ...
        display_text = "\n".join(
            [
                f"{v['name']}: {v['dist']}({', '.join(map(str, v['params']))})"
                for v in vars_data
            ]
        )
...
def run_simulation(n_clicks, n_iter, seed, formula, vars_data):
    ...
    n_iter : Any
        Input value for ``n_iter``.
    seed : Any
        Input value for ``seed``.
    formula : Any
        Input value for ``formula``.
    vars_data : Any
        Value provided for this parameter."""
    ...
    try:
        ...
        for var in vars_data:
            ...
            if dist == "normal":
                ...
                samples[name] = np.random.triangular(
                    params[0], params[1], params[2], n_iter
                )
            elif dist == "lognormal":
                samples[name] = np.random.lognormal(
                    np.log(params[0]), params[1] / params[0], n_iter
                )
            else:  # constant
        ...
        dist_fig.add_vline(
            x=median, line_dash="dash", line_color="green", annotation_text="Median"
        )
        ...
        percentiles_fig.add_hline(
            y=mean, line_dash="dash", line_color="red", annotation_text="Mean"
        )
