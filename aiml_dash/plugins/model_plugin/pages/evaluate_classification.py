                                    ...
                                    dmc.TabsPanel(
                                        [
                                            dmc.Card(
                                                [
                                                    dcc.Graph(
                                                        id="evalclass-distribution-plot"
                                                    ),
                                                ],
                                                withBorder=True,
                                                radius="md",
...
def evaluate_classification(
    n_clicks, dataset_name, actual_col, pred_col, proba_col, compare_cols
):
    ...
    actual_col : Any
        Input value for ``actual_col``.
    pred_col : Any
        Input value for ``pred_col``.
    proba_col : Any
        Input value for ``proba_col``.
    compare_cols : Any
        Value provided for this parameter."""
            ...
            for col in compare_cols:
                if col != pred_col and col in df.columns:
                    ...
                    if len(y_comp) > 0:
                        ...
                        if is_binary:
                            f1_comp = f1_score(
                                y_act_comp, y_comp, average="binary", zero_division=0
                            )
                        else:
                            f1_comp = f1_score(
                                y_act_comp, y_comp, average="weighted", zero_division=0
                            )
                        compare_data.append(
                            {
                                "Model": col,
...
def export_evaluation(n_clicks, dataset_name, actual_col, pred_col):
    ...
    try:
        ...
        return dcc.send_data_frame(
            results_df.to_csv, "classification_evaluation.csv", index=False
        )
