...
def calculate_ev(node):
    ...
    if node.get("type") == "chance":
        ev = sum(
            branch.get("prob", 1) * calculate_ev(branch)
            ...
            results = dmc.Stack(
                [
                    ...
                    *[
                        ...
                        for b in sorted(
                            branch_evs, key=lambda x: x["Expected Value"], reverse=True
                        )
                    ],
                ],
                gap="xs",
