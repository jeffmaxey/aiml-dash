...
def layout():
    """Create the random forest page layout."""
            ...
            dmc.Tabs(
                [
                    ...
                    dmc.TabsPanel(
                        [
                            dmc.Card(
                                [html.Div(id="rf-summary")], withBorder=True, p="md"
                            ),
                        ],
                        value="summary",
                    ),
...
def update_variables(dataset):
    """Update response and explanatory variable options for a dataset.
...
def train_forest(n, dataset, response, explanatory, rf_type, n_trees, max_depth, seed):
    """Train a random forest model and return model metadata, results, and status UI.
    ...
    rf_type : Any
        Input value for ``rf_type``.
    n_trees : Any
        Input value for ``n_trees``.
    max_depth : Any
...
def update_summary(results):
    """Render summary metrics for the most recently trained forest.
...
def update_plot(results):
    ...
    fig = go.Figure(
        go.Bar(x=list(importance.values()), y=list(importance.keys()), orientation="h")
    )
    fig.update_layout(
