                                    ...
                                    dmc.Stack(
                                        [
                                            ...
                                            dmc.Button(
                                                ...
                                                leftSection=DashIconify(
                                                    icon="carbon:recommend", width=20
                                                ),
                                                fullWidth=True,
                                                color="green",
                                            ),
...
def build_model(
    n_clicks, dataset_name, user_col, item_col, rating_col, method, n_neighbors
):
    ...
    user_col : Any
        Input value for ``user_col``.
    item_col : Any
        Input value for ``item_col``.
    rating_col : Any
        Input value for ``rating_col``.
    method : Any
    ...
    try:
        ...
        matrix = df.pivot_table(
            values=rating_col, index=user_col, columns=item_col, fill_value=0
        )
        ...
        if method == "user":
            ...
            for item in matrix.columns:
                if user_ratings[item] == 0:  # Only recommend unrated items
                    ...
                    if mask.any():
                        weighted_rating = (
                            similar_ratings[mask] * weights[mask]
                        ).sum() / weights[mask].sum()
