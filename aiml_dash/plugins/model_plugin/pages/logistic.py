                                ...
                                [
                                    dmc.TabsList(
                                        [
                                            ...
                                            dmc.TabsTab(
                                                "Confusion Matrix", value="confusion"
                                            ),
...
def populate_datasets(_):
    ...
    return [{"value": name, "label": name} for name in data_manager.get_dataset_names()]
...
def populate_variables(dataset_name):
    """Populate response and predictor options for the selected dataset.
...
def estimate_model(n_clicks, dataset_name, response, predictors):
    """Estimate logistic regression model and return summary artifacts.
    ...
    try:
        ...
        model_store = {
            "coefficients": {
                col: float(coef)
                for col, coef in zip(X.columns, model.coef_[0], strict=False)
            },
                    ...
                    [
                        ...
                        html.Tbody(
                            [
                                ...
                                html.Tr(
                                    [html.Td("Precision"), html.Td(f"{precision:.4f}")]
                                ),
...
def update_plot(plot_type, model_store):
    """Render ROC or coefficient plot for the estimated logistic model.
    ...
    if plot_type == "roc":
        ...
        fig = go.Figure(
            data=[go.Bar(x=list(coefs.values()), y=list(coefs.keys()), orientation="h")]
        )
        fig.update_layout(
            title="Coefficient Plot", xaxis_title="Coefficient", yaxis_title="Variable"
        )
